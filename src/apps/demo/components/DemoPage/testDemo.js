...
class testDemo extends Component {
	...
	getInterpolatedData = event => {
		...
		console.log("Result is here: "+ JSON.stringify(this.state.result))
			...
			if (annotIncidentLength === 1) {
				...
				for (let j = 0; j < annotIncidentLength - 1; j++) {
					...
					for (let k=sFrame+1; k<=tFrame; k++){
						...
						let interpolH = source.height + (ySlope * off)
						console.log("for k= "+k+"interpolated X value= "+interpolX)
						console.log("for k= "+k+"interpolated Y value= "+interpolY)
						console.log("for k= "+k+"interpolated W value= "+interpolW)
						console.log("for k= "+k+"interpolated H value= "+interpolH)
						interpolatedResult.annotations[i].incidents.splice(j + 1, 0, ({
							id: `${timeNow}`, name: `${timeNow}`, x: interpolX, y: interpolY, height: interpolH, width: interpolW, time: k, status,
						}));
					}
